...
# Officers by allotment year
profile %>%
  filter(!is.na(Allotment_Year), Allotment_Year >= 2000) %>%
  count(Allotment_Year) %>%
  ggplot(aes(x = Allotment_Year, y = n)) +
  geom_line(color = "steelblue", size = 1) +
  geom_point(color = "steelblue") +
...
1. Profile + Education → Get officer background
2. Result + Experience → Get career with background
...
2. OR focus on IAS careers (backgrounds → promotions)
...
cat("✓ Analysis dataset saved to: data/analysis/dc_with_backgrounds.rds\n")
cat("✓ This file contains", nrow(dc_analysis), "DC positions with background data\n")
```
